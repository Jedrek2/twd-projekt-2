...
dane <- read.csv("goodreads_library_export.csv",sep = ";",header = TRUE)
...
oceny <- przeczytane %>%
  ...
  filter(!is.na(my_rating),
         !is.na(avg_rating))
df<-read.csv("wiadomosci.csv")
df2<-read.csv("wiadomosci_przetworzone2.csv")
x <- theme(
...
html <- read_html("historia.java", encoding = "UTF-8")
...
server <- function(input, output, session){
  ...
  output$wykres2j <- renderPlotly({
    ...
    p <- ggplot(d, aes(date)) +
      geom_col(aes(y = n), fill = "#D0D7DE", width = 0.9) +
      geom_line(aes(y = ma7), color = "#1F77B4", linewidth = 1.1) +
      scale_x_date(date_breaks = "1 month", date_labels = "%Y-%m") +
      labs(
        title = "Liczba zapytań dziennie",
        subtitle = "linia = średnia krocząca 7 dni",
        x = NULL,
        y = "Liczba zapytań"
      ) +
